...
```{r}
gastric_nsaf <- as.matrix(read.csv('missing-protein-project/data/gastric-primary-cell/gastric_primary_cell_nsaf.csv', row.names = 1))
gastric_count <- read_csv('missing-protein-project/data/gastric-primary-cell/gastric_primary_cell_count_2.csv', show_col_types = FALSE)
colnames(gastric_count)[1] <- 'accession'
omics_metadata <- read_csv('missing-protein-project/data/support-data/omics_metadata.csv', show_col_types = FALSE)
load('missing-protein-project/cache/missing_protein.rda')
```
...
detected_MP_proteins <- combined_long %>% filter(!(IPAS %in% c('IP0981_1701', 'IP7103_1701'))) %>%  filter(accession %in% missing_proteins) %>% filter(NSAF > 0  & TPM > 0) %>% arrange(accession) %>% group_by(accession) %>% summarise(n=n()) %>% filter(n >= 2)
hist(detected_MP_proteins$n)


# check the protein and RNA expression for the detected missing proteins
full_table_detected_MP <- combined_long %>%  filter(!(IPAS %in% c('IP0981_1701', 'IP7103_1701'))) %>% filter(accession %in% missing_proteins) %>% filter(NSAF > 0 & TPM > 0) %>% arrange(accession, IPAS) %>% filter(accession %in% detected_MP_proteins$accession)
...
write_csv(full_table_detected_MP, 'output/protein list/detected_missing_protein.csv')
...
expressed_regular_proteins <- combined_long %>% filter(!(IPAS %in% c('IP0981_1701', 'IP7103_1701'))) %>%  filter(!(accession %in% P2_5_proteins)) %>% filter(NSAF > 0) %>% arrange(accession) %>% group_by(accession) %>% summarise(n=n()) %>% filter(n >= 6)
