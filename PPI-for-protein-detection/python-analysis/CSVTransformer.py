...
parser.add_argument('protein-data',  type=str,
...
parser.add_argument('-d', '--identifier-addition-proteinData', type=str, default='Entry',
                    help='a string for indicating the unique identifier in the additionalProteinData file')

parser.add_argument('-f', '--output-file-name', type=str,
...
protein_data_id = args.identifierProteinData
additional_data_id = args.identifierAdditionProteinData

# import data
protein_data = pd.read_csv(args.proteinData)

if args.additionalProteinData is not None:
    additional_protein_data = pd.read_table(args.additionalProteinData, sep='\t')
...
if args.outputFileName is not None:
    protein_data.to_csv(args.args.outputFileName)
else:
    path = args.proteinData + '_process.csv'
