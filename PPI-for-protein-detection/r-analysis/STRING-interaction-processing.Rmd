...
```{r}
nextprot_proteins <- readxl::read_xls('data/nextprot_protein_entries.xls')
...
write_csv(nextprot_proteins, file = 'data/nextprot-proteins.csv')
```
...
```{r}
all_proteins_0981 <- read_csv('data/raw-data/combined-protein/combined_protein_7103.csv', show_col_types = FALSE)
all_proteins_0981 <- all_proteins_0981 %>% janitor::clean_names()
# all_proteins_0981 <- all_proteins_0981 %>% select(protein_accession, protein_data_base_type, protein_score, protein_matched_peptides,  protein_matched_peptide_inten_sum)
```
...
swiss_proteins_short_unique <- swiss_proteins_0981_short %>% group_by(protein_accession) %>% arrange(protein_score) %>% filter(row_number() == 1)
...
```{r}
write_csv(swiss_proteins_short_unique, file='data/proteins-sample-7103.csv')
```
...
```{r}
gastric_nsaf <- read.csv('PPI-for-protein-detection/data/gastric-primary-cell/gastric_primary_cell_nsaf.csv')
```
...
```{r}
expressed_proteins <-  gastric_nsaf %>% select(-c(IP0981_1701, IP7103_1701)) %>% mutate(across(IP0982_1701:IP7105_1701, ~ .x == 0)) %>% rowwise() %>% 
  mutate(num_zeros=sum(c_across(IP0982_1701:IP7105_1701))) %>% filter(num_zeros >= 3) %>% select(X) %>% pull()
```
```{r}
expressed_proteins_swiss_prot <- expressed_proteins[expressed_proteins %in% nextprot_proteins$acc_code]
```


```{r}
write_csv(tibble(accession=expressed_proteins_swiss_prot), file = 'PPI-for-protein-detection/data/expressed-protein.csv')
```
