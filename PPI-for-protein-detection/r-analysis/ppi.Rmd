...
```{r}
ppi <- read.delim('data/BioGRID/BioGRID-ORGANISM-Homo_sapiens-4.4.217.mitab.txt')
ppi_short <- ppi[1:100, ]
head(ppi_short)
...
```{r}
gastric_nsaf <- read.csv('data/gastric-primary-cell/gastric_primary_cell_nsaf.csv', row.names = 1)
...
unique_protein_dat <- dat %>% group_by(protein_accession) %>% arrange(FDR) %>% filter(row_number() == 1) %>% filter(protein_data_base_type=='Regular')
write_csv(unique_protein_dat, 'data/protein_scores.csv')
```
