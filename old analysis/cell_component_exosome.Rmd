...
```{r}
# Reading data
SpC <- read.csv("data/CellLineCompartment_Exosome_SumTickNormalization.csv", skip=2, row.names = 'Accession')
...
```{r}
unique_DEPs_exosome <- unique(unlist(lapply(res_lst, rownames)))
...
# Visulization
```{r}
plot_heat_map(y, upper_limit = 5, row_keep = unique_DEPs_exosome, show_rownames = F, main='Cell Component Exosome')
```
